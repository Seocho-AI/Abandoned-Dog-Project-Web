...
class BreedsDataFeatureProcessor():
    ...
    def __init__(self, target_user_id: str, breeds_panel, user_survey_data, adopter_data, dog_list_data, breed_info):
        ...
        self.user_dog_experience_num_dict = {
            key: int(5 * (key / 100)) for key in range(100)
        }
        self.user_dog_experience_time_dict = {
            key: int(5 * (key / 240)) for key in range(240)
        }
    ...
    def processing_user_survey_data_features(self):
        ...
        self.processed_user_data['c3_easy_to_groom'] = 5 - \
            self.processed_user_data['c1_amount_of_shedding']
    ...
    def processing_user_age(self):
        ...
        self.adopter_scaled = pd.DataFrame(
            adopter_scaled, columns=adopter.columns)
    ...
    def processing_dog_list_data_features(self):
        ...
        dog_list_data = self.dog_list_data.loc[(
            self.dog_list_data['neuterYn'] == self.user_survey_data['neuter_yn'])]
        # print(dog_list_data)
        # print(self.breed_info[self.breed_info['size'] == '중형견']['breed_name_kr'])
        # if self.user_survey_data['dog_size'] == '소형견':
        #     dog_list_data = dog_list_data.loc[dog_list_data['kindCd'] == self.breed_info[self.breed_info['size'] == '소형견']['breed_name_kr']]
        # if self.user_survey_data['dog_size'] == '중형견':
        #     dog_list_data = dog_list_data.loc[dog_list_data['kindCd'] == self.breed_info[self.breed_info['size'] == '중형견']['breed_name_kr']]
        # if self.user_survey_data['dog_size'] == '대형견':
        #     dog_list_data = dog_list_data.loc[dog_list_data['kindCd'] == self.breed_info[self.breed_info['size'] == '대형견']['breed_name_kr']]
        #
        ...
        if self.user_survey_data['want_dog_age'] == '자견(생후 2년 이하)':
            dog_list_data = dog_list_data.loc[(dog_list_data['age'] == '2022(년생)') | (
                dog_list_data['age'] == '2021(년생)')]
        else:
            dog_list_data = dog_list_data.loc[(dog_list_data['age'] != '2022(년생)') & (
                dog_list_data['age'] != '2021(년생)')]
        ...
        for i in dog_list_data['desertionNo'].values:
            breed_kr = dog_list_data.loc[dog_list_data['desertionNo']
                                         == i, 'kindCd'].values[0]
            # print(breed_kr)
            if breed_kr not in self.breed_info['breed_name_kr'].values:
                for key in panel:
                    ...
                    # print(panel_rating)
                    dog_list_data.loc[dog_list_data['desertionNo']
                                      == i, key] = 3
                continue
            breed_en = self.breed_info.loc[self.breed_info['breed_name_kr']
                                           == breed_kr, 'breed_name'].values[0]
            # print(breed_en)
            ...
            for key in panel:
                ...
                panel_rating = self.breeds_panel.loc[self.breeds_panel['breed']
                                                     == breed_en, key]
                # print(panel_rating)
                dog_list_data.loc[dog_list_data['desertionNo']
                                  == i, key] = panel_rating.values[0]
